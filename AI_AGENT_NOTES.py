...
VERSION_HISTORY = """
[2.7.8] - 2026-04-17
  -  智慧型腳本拼接：實作自動時間戳回溯偵測，支援錄製片段與文字指令無縫組合。
  -  執行引擎修復：解決「延遲指令」導致的進度追趕問題，確保隨機延遲後腳本節奏正常。
  -  AI 指令擴展：支援 AI: 前綴語法，可在 if 指令中直接呼叫 YOLO 偵測。
  -  擬真滑鼠支援：新增 >擬真滑鼠>開啟 指令，支援文字控制貝茲曲線移動。
  -  指令邏輯統一：完成 Trigger、Parallel、StateMachine 的核心執行路徑統一化。
  -  修復 Retry 邏輯：解決 try_action 在複雜分支下的跳轉偏移問題。
...
LOCALIZATION_RULES = """
所有針對使用者的顯示文字（UI 按鈕、標籤、訊息、日誌、說明文件）必須嚴格遵守「台灣在地化」規範。
...
【開發執行規範】
- 撰寫程式碼時，若涉及顯示文字，應優先查閱 `lang.py` 是否已有對應翻譯。
- 若需新增翻譯，必須確保 `繁體中文` Key 及其對應值皆為台灣用語。
- AI Agent 在修改現有代碼時，若發現非在地化用語，應順手修正。
"""
...
REPORT_FILE_MANAGEMENT_RULES = """
為減少資料夾中重複或過多的報告檔案（如 report.md, summary.md 等），請遵守以下規範：

1. **優先檢查現有檔案**：
   - 在建立新的報告檔案（如 .agent/task_report.md）之前，先檢查該目錄下是否已存在同類型的報告檔案。
   - 如果存在，**請直接讀取並更新該檔案內容**，添加新的章節或資訊，而不是建立新的檔案（如 task_report_v2.md）。

2. **統一報告名稱**：
   - 儘量使用固定的檔案名稱，例如：
     - `.agent/TASK_PROGRESS.md`（任務進度與紀錄）
     - `.agent/DEBUG_LOG.md`（除錯紀錄）
   - 避免使用帶有時間戳記或隨機後綴的檔案名稱（除非是必要的備份）。

3. **更新而非新建**：
   - 當需要回報進度或結果時，將新內容附加（Append）到現有報告的尾端，並加上時間戳記或分隔線。
   - 範例格式：
     ```markdown
     
     ## [2025-12-06 10:00:00] 更新說明
     - 完成了 XXX 功能
     - 修復了 YYY 問題
     ```

4. **定期清理**：
   - 若某些臨時報告檔案已無使用價值，請在任務結束時自動刪除。
"""
...
12. 遵守【報告檔案管理規範】，優先更新現有報告檔案，避免產生過多垃圾檔案
13. 所有顯示文字必須符合【台灣在地化用語規範】，禁止使用簡體字及支語
"""
