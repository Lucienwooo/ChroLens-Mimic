...
import threading, time, json, os, datetime, multiprocessing
...
class OCRFloatWindow:
    ...
    def show_result(self, image_np, text, main_x, main_y):
        ...
        if len(image_np.shape) == 2:
            rgb_np = cv2.cvtColor(image_np, cv2.COLOR_GRAY2RGB)
        elif image_np.shape[2] == 4:
            rgb_np = cv2.cvtColor(image_np, cv2.COLOR_BGRA2RGB)
        else:
...
class VisualDetectionEngine:
    ...
    def recognize_text(self, img, is_captcha=True):
        ...
        try:
            ...
            if is_captcha and (DDDDOCR_AVAILABLE or OCR_ENGINE_TYPE == 'ddddocr'):
                # 取得 ddddocr 實例
                ...
                if hasattr(self, '_ddddocr_instance') and self._ddddocr_instance is not None:
                    # 【策略一】直接對原始影像進行 ddddocr
                    try:
                        success, encoded_image = cv2.imencode('.png', img)
                        if success:
                            res = self._ddddocr_instance.classification(encoded_image.tobytes()).strip()
                            if 3 <= len(res) <= 7:
                                return res
                    except:
                        pass
                    ...
                    try:
                        ...
                        success, encoded_image = cv2.imencode('.png', resized_gray)
                        if success:
                            res = self._ddddocr_instance.classification(encoded_image.tobytes()).strip()
                            if 3 <= len(res) <= 7:
                                return res
            ...
            if is_captcha:
                # 方案一：地端影像預處理強化實作
                # A. 雙倍放大影像（雙立方插值），使邊緣與文字輪廓更加平滑與連續
                resized = cv2.resize(img, None, fx=2, fy=2, interpolation=cv2.INTER_CUBIC)
                
                # B. 轉灰階
                gray = cv2.cvtColor(resized, cv2.COLOR_BGR2GRAY)
                
                # C. 雙邊濾波 (Bilateral Filter) 去噪，同時保留清晰的邊緣（避免模糊掉字體邊界）
                denoised = cv2.bilateralFilter(gray, 9, 75, 75)
                
                # D. 大津法自適應二值化 (Otsu Thresholding) 提取字體
                _, thresh = cv2.threshold(denoised, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)
                
                # E. 進行形態學膨脹/侵蝕 (Morphological Closing) 融合字體空隙並抹除干擾線
                kernel = np.ones((2, 2), np.uint8)
                dilated = cv2.dilate(thresh, kernel, iterations=1)
                
                # F. 連通區域分析 (Connected Components) 抹除極小像素噪點
                num_labels, labels, stats, centroids = cv2.connectedComponentsWithStats(dilated)
                cleaned = np.zeros_like(dilated)
                for i in range(1, num_labels):
                    if stats[i, cv2.CC_STAT_AREA] > 15:  # 過濾掉面積小於15像素的雜訊
                        cleaned[labels == i] = 255
                
                # G. 顏色反向 (Invert) 轉換為白底黑字（這是 ddddocr 與 Tesseract 辨識度最高的格式！）
            ...
            # 2. 執行辨識
            if is_captcha and (DDDDOCR_AVAILABLE or OCR_ENGINE_TYPE == 'ddddocr'):
                # 策略三：最後退回至高強度預處理圖的 ddddocr 辨識
                if not hasattr(self, '_ddddocr_instance') or self._ddddocr_instance is None:
                    import ddddocr
                    self._ddddocr_instance = ddddocr.DdddOcr(show_ad=False)
                success, encoded_image = cv2.imencode('.png', processed)
                if success:
                    res = self._ddddocr_instance.classification(encoded_image.tobytes())
                    return res.strip()

            # 對於非驗證碼（如按鈕），優先使用 cnocr
            if CNOCR_AVAILABLE or OCR_ENGINE_TYPE == 'cnocr':
                if not hasattr(self, '_cnocr_instance') or self._cnocr_instance is None:
                    from cnocr import CnOcr
                    self._cnocr_instance = CnOcr()
                
                # 單行文字辨識
                res = self._cnocr_instance.ocr_for_single_line(img)
                text = res.get('text', '').strip() if res else ''
                
                # 降級嘗試：在灰度圖上進行辨識
                if not text or res.get('score', 0) < 0.2:
                    gray_img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
                    res_gray = self._cnocr_instance.ocr_for_single_line(gray_img)
            ...
            elif OCR_ENGINE_TYPE == 'tesseract' or TESSERACT_AVAILABLE:
                import pytesseract
...
LAST_SCRIPT_FILE = os.path.join(_dir, "last_script.txt")
LAST_SKIN_FILE = os.path.join(_dir, "last_skin.txt")  # 新增這行
...
class RecorderApp(tb.Window):
    def __init__(self):
        ...
        if not is_admin():
            # 顯示警告但仍繼續執行
            try:
                print("[WARN] 警告：程式未以管理員身份執行，錄製功能可能無法正常工作！")
            except:
        ...
        self.visual_anchor_enabled = tk.BooleanVar(value=True)  # 全局視覺錨點開關
    ...
    def merge_scripts(self):
        ...
        #  使用列表儲存合併資料，支援重複腳本（每個元素：{"name": script_name, "delay": delay_seconds}）
    ...
    def _refresh_target_window(self, event=None):
        """重新整理目標視窗（可由左鍵點擊觸發）- 以相同視窗名稱重新指定"""
        if not self.target_title:
            self.log("沒有目標視窗可重新整理")
            return
...
# ====== 設定檔讀寫 ======
CONFIG_FILE = os.path.join(_dir, "user_config.json")
...
if __name__ == "__main__":
    # 🔧 PyInstaller 支援：必須在主程式入口呼叫 freeze_support()
    # 這是因為 YOLO (ultralytics) 和其他模組可能使用 multiprocessing
