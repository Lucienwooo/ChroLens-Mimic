...
import json
import os
from utils.db_manager import GalleryDBManager
...
from parser.editor_parser import EditorParserMixin
from ui.editor_flowchart import EditorFlowchartMixin
class TextCommandEditor(EditorParserMixin, EditorFlowchartMixin, tk.Toplevel):
    """文字指令式腳本編輯器"""
    ...
    def on_close_editor(self):
        if self.parent and hasattr(self.parent, "user_config"):
            self.parent.user_config["editor_geometry"] = self.geometry()
            try:
    ...
    def _create_ui(self):
        ...
        # ENG mode
        self.eng_mode_var = tk.BooleanVar(value=False)
        eng_mode_check = tk.Checkbutton(
            trajectory_control,
            text="ENG mode",
            variable=self.eng_mode_var,
            command=self._toggle_eng_mode,
            font=font_tuple(9)
        )
        eng_mode_check.pack(side="left", padx=15)
        ...
        self.text_editor.tag_config("syntax_wait", foreground="#FFFF00")        # 亮黃色 - 等待關鍵字
        ...
        self.module_preview.tag_config("syntax_wait", foreground="#FFFF00")
        ...
        self.module_preview.tag_config("syntax_key_ctrl", foreground="#569cd6")
        self.module_preview.tag_config("syntax_key_alt", foreground="#dcdcaa")
        self.module_preview.tag_config("syntax_key_shift", foreground="#4ec9b0")
        self.module_preview.tag_config("syntax_key_plus", foreground="#d4d4d4")
        self.module_preview.tag_config("syntax_key_normal", foreground="#c586c0")
    ...
    def _show_message(self, title, message, msg_type="info"):
        """顯示自訂訊息對話視窗，不會改變父視窗位置"""
        dialog = tk.Toplevel(self)
        set_window_icon(dialog)
        dialog.title(title)
        dialog.transient(self)  # 設定為編輯器的子視窗
        dialog.grab_set()  # 模態對話視窗
    ...
    def _convert_text_to_canvas(self, text_content):
        ...
        try:
            from parser.command_lang import translate_script_line_to_canonical
    ...
    def _create_command_buttons_in_frame(self, parent_frame):
        ...
        self.button_categories = {
            "圖片辨識": [
                ...
                ("移動至圖片", "#673AB7", None, ">移動至>pic01, T=1s500"),
                ("點擊圖片", "#3F51B5", None, ">左鍵點擊>pic01, T=1s500"),
                ("條件判斷", "#2196F3", None, ">if>pic01, T=1s500\n>>#標籤\n>>>#標籤"),
                ("等待圖片", "#1976D2", None, ">等待圖片>pic01, 等待T=0s000, T=1s500"),
            ...
            "滑鼠鍵盤": [
                ...
                ("按下按鍵", "#8BC34A", self._capture_keyboard, None),
        ...
        self.after(100, lambda: self._switch_category(self.current_category))
    ...
    def _insert_command_template(self, template):
        """插入指令模板並套用內容感知 (v2.8)"""
        try:
            from parser.command_lang import translate_ui_string
            lang = getattr(self.parent, "actual_language", "繁體中文")
            template = translate_ui_string(template, lang)
        except ImportError:
            pass
        cursor_pos = self.text_editor.index(tk.INSERT)
        
        # 內容感知：自動尋找上一個圖片名稱
        if "pic01" in template:
            try:
                content = self.text_editor.get("1.0", tk.INSERT)
                # 尋找最近的 picXXX 名稱
                pic_matches = re.findall(r'>(?:辨識|移動至|if|左鍵點擊|右鍵點擊)>(pic[\w\d]+)', content)
                if pic_matches:
                    last_pic = pic_matches[-1]
                    template = template.replace("pic01", last_pic)
            except:
                pass
        ...
        if ">放開a" in template:
            try:
                content = self.text_editor.get("1.0", tk.INSERT)
                # 尋找最近的按下按鍵
                key_matches = re.findall(r'>按下([^,]+)', content)
                if key_matches:
                    last_key = key_matches[-1].strip()
                    template = template.replace(">放開a", f">放開{last_key}")
            except:
                pass
    ...
    def _configure_reference_tags(self, text_widget):
        ...
        text_widget.tag_config("syntax_wait", foreground="#FFFF00")        # 亮黃色 - 等待關鍵字
    ...
    def _insert_command_grid_table(self, parent_frame):
        ...
        commands = [
            ...
            ("等待圖片", ">等待圖片>pic01, 等待T=0s000, T=1s500", "暫停執行直到指定圖片出現"),
    ...
    def _insert_command_with_syntax_highlight(self, text_widget, command):
        ...
        while pos < len(command):
            if command[pos] == '>':
                ...
                elif keyword in ['重複', '重複結束', '當', '迴圈結束', '隨機執行']: tag = "syntax_flow"
    ...
    def _format_combat_event(self, event: dict) -> str:
        ...
        # 迴圈攻擊
            ...
            return f"迴圈攻擊[{', '.join(templates)}], 攻擊鍵{attack_key}, 間隔{interval}s"
    ...
    def _format_branch_action(self, branch: dict) -> str:
        ...
        if action == "continue":
            ...
            #  優化顯示：999999視為無限迴圈，不顯示次數；其他次數才顯示
            ...
            if re.match(r'^>#mod_([a-zA-Z0-9_\u4e00-\u9fa5]{1,30})$', stripped):
                ...
                if os.path.exists(module_path):
                    try:
                        ...
                        # 載入失敗，保留原始引用並添加註釋
                        expanded_lines.append(f"{line}  # 模組載入失敗: {e}")
                else:
                    # 模組不存在，保留原始引用並添加註釋
    ...
    def _apply_syntax_highlighting_to_widget(self, text_widget):
        ...
        try:
            ...
            # 加入英文翻譯對應的 Patterns
            try:
                from parser.command_lang import COMMAND_MAP_EN
                eng_patterns = []
    ...
    def _toggle_eng_mode(self):
        if self.eng_mode_var.get():
            self._translate_script_to_lang("English")
        else:
            lang = getattr(self.parent, "actual_language", "繁體中文")
            self._translate_script_to_lang(lang)
            
    def _translate_script_to_current_lang(self):
        lang = getattr(self.parent, "actual_language", "繁體中文")
        self._translate_script_to_lang(lang)

    def _translate_script_to_lang(self, lang):
        try:
            from parser.command_lang import translate_ui_string, translate_script_line_to_canonical
            ...
            translated_lines = []
            for line in lines:
                zh_line = translate_script_line_to_canonical(line)
                target_line = translate_ui_string(zh_line, lang)
            ...
            if hasattr(self, "_original_text_insert"):
                self._original_text_insert("1.0", new_content)
            else:
                self.text_editor.insert("1.0", new_content)
                
            self._apply_syntax_highlighting()
        except Exception as e:
            self._show_message("Error", f"Failed to translate script: {e}", "error")
    ...
    def _execute_script(self):
        ...
        # 確保不在錄製或播放狀態 (移到最前面，防止覆蓋正在執行中的事件)
        ...
        try:
            ...
            # (移除對 core_recorder.events 的直接修改，因為 ChroLens_Mimic.py 的 _continue_play_record() 會處理)
            ...
            if hasattr(self.parent, 'target_hwnd') and self.parent.target_hwnd:
                from utils.utils import get_window_info
...
"""
    ...
    def _capture_keyboard(self):
        dialog = KeyCaptureDialog(self, lambda key: self._insert_command_template(f">按下{key}, T=0s500"))

    def _capture_relative_move(self):
        """啟動相對移動距離測量工具"""
        if self.script_var.get() == "":
            messagebox.showwarning("警告", "請先選擇或建立一個腳本")
            return
            
        def on_complete(dx, dy):
            command = f">相對移動({dx},{dy}), T=0s000\n"
        ...
        RelativeMoveDistanceTool(self, on_complete)
    ...
    def _on_coordinate_captured(self, coordinate, button_type):
        ...
        try:
            ...
            if button_type == "left":
                x, y = coordinate
                command = f">左鍵點擊({x},{y}), T={current_time}\n"
            elif button_type == "right":
                x, y = coordinate
                command = f">右鍵點擊({x},{y}), T={current_time}\n"
            elif button_type == "drag":
                points = coordinate["points"]
                durations = coordinate["durations"]
                start_x, start_y = points[0]
                
                # 起點移動與按下
                lines = [
                    f">移動至({start_x},{start_y}), T={current_time}",
                    f">按下左鍵, T=0s050"
                ]
                # 中繼與終點移動
                for idx in range(1, len(points)):
                    px, py = points[idx]
                    dur_s = durations[idx - 1]
                    formatted_dur = self._format_time(dur_s)
                    lines.append(f">移動至({px},{py}), T={formatted_dur}")
                # 放開左鍵
                lines.append(f">放開左鍵, T=0s000")
            ...
            # 更新狀態列
            if button_type == "drag":
                self._update_status(f"拖曳軌跡已插入：共 {len(points)} 個點，總用時 {sum(durations):.2f} 秒", "success")
            else:
                button_name = "left" if button_type == "left" else "right" if button_type == "right" else "click"
                button_name = "左鍵" if button_name == "left" else "右鍵" if button_name == "right" else "點擊"
    ...
    def _do_ocr_input_region_selection(self):
        ...
        try:
            # 創建範圍選擇視窗
            from ui.text_script_editor import RegionSelector
    ...
    def _on_relative_ocr_selected(self, region):
        ...
        try:
            from vision.ocr_trigger import OCRTrigger
    ...
    def _get_pcb_node_style(self, name, node_type):
        ...
        elif node_type == "thread" or '[執行緒:' in name:
            return {"icon": "∥", "icon_color": "#a371f7", "border": "#a371f7"}  # 紫色
    ...
    def _draw_workflow_node(self, label, commands):
        ...
        node_data['radius'] = radius  # 儲存半徑供路徑計算使用
    ...
    def _add_grid_row(self, data, index=None, is_add_row=False):
        ...
        cat_combo = ttk.Combobox(row_frame, textvariable=cat_var, values=["影像辨識", "滑鼠鍵盤", "流程控制", "註解", "空白"], width=10, state="readonly", font=font_tuple(9))
...
class ImageGalleryViewer(tk.Toplevel):
    def __init__(self, parent, editor):
        ...
        self.db = GalleryDBManager(os.path.join(self.images_root, ".gallery.db"))
        self.db.sync_directory(self.images_root)
        ...
        self.confirm_move_btn = tb.Button(sort_frame, text="確認移動", bootstyle="success", state="disabled", command=self._confirm_batch_move)
        self.confirm_move_btn.pack(side="right", padx=5)
        
        self.batch_move_btn = tb.Button(sort_frame, text="批次移動", bootstyle="primary", command=self._toggle_batch_mode)
        self.batch_move_btn.pack(side="right", padx=5)
    ...
    def _toggle_sort(self, sort_type):
        if self.sort_by == sort_type:
            ...
            # 預設: 名稱排序預設為順序(A->Z)，時間排序預設為由新到舊(降冪)
            self.sort_asc = True if sort_type == "name" else False
    ...
    def _on_folder_select(self, event):
        ...
        if selection:
            ...
            if getattr(self, "is_batch_mode", False):
                self.target_move_folder = "" if item == "全部圖片" else item
                self.status_var.set(f"批次目標設為 [{item}]，請點擊「確認移動」。")
                return
    ...
    def _load_images(self):
        ...
        folder = self.current_folder if self.current_folder is not None else None
        keyword = self.search_var.get().lower()
        
        db_files = self.db.search_images(folder, keyword)
        
        def sort_key(item):
            full_p = os.path.join(self.images_root, item['rel_path'])
        ...
        db_files.sort(key=sort_key, reverse=not is_asc)
        
        files = [item['rel_path'] for item in db_files]
        ...
        cache_dir = os.path.join(self.images_root, ".thumbnails")
        ...
        for f in files:
            try:
                ...
                # Placeholder Label
                img_lbl = tk.Label(frame, text="Loading...", bg="#1e1e1e", fg="#888888", width=15, height=7, cursor="hand2")
                img_lbl.pack()
                
                # Display name without extension and without "pic" prefix
                display_name = os.path.splitext(os.path.basename(f))[0]
                if display_name.lower().startswith("pic"):
                    display_name = display_name[3:]
                    
                name_lbl = tk.Label(frame, text=display_name, bg="#2d2d2d", fg="#ffffff", font=font_tuple(9), cursor="hand2")
                name_lbl.pack(pady=5)
                
                def on_click(event, fname=os.path.basename(f), fpath=img_path):
                    if getattr(self, "is_batch_mode", False):
                        if not hasattr(self, "selected_batch_images"):
                            self.selected_batch_images = set()
                        if fpath in self.selected_batch_images:
                            self.selected_batch_images.remove(fpath)
                        else:
                            self.selected_batch_images.add(fpath)
                        self._update_batch_ui()
                        return
                ...
                # Submit thumbnail load to background
                self.executor.submit(self._load_single_thumbnail, f, img_path, img_lbl, current_load_id, cache_dir)
    ...
    def _load_single_thumbnail(self, rel_path, img_path, img_lbl, load_id, cache_dir):
        if self.load_id != load_id:
            return
        ...
        try:
            ...
            if os.path.exists(cache_path):
                if os.path.getmtime(cache_path) >= os.path.getmtime(img_path):
                    use_cache = True
            ...
            if use_cache:
                ...
                if hasattr(img, 'draft'):
                    img.draft('RGB', (120, 120))
                img.thumbnail((120, 120))
                if img.mode != 'RGB' and img.mode != 'RGBA':
                    img = img.convert('RGBA')
                img.save(cache_path, format="PNG")
                
            # Keep photo reference strictly local for now, creating PhotoImage must happen in main thread!
            # Wait, ImageTk.PhotoImage SHOULD be created in main thread in Tkinter.
            # So we pass the PIL Image to the main thread.
            
            def update_ui(loaded_img=img):
                if self.load_id == load_id and img_lbl.winfo_exists():
                    try:
            ...
            def update_err():
                if self.load_id == load_id and img_lbl.winfo_exists():
                    img_lbl.configure(text="Error", fg="red")
